...
pub trait DBClient: Send + Sync + 'static {
    async fn insert_entity(&self, id: Uuid) -> Result<(), DBError>;

    async fn get_entity(&self, id: Uuid) -> Result<Entity, DBError>;
}
...
impl DBClient for PostgresDBClient {
    ...
    async fn insert_entity(&self, id: Uuid) -> Result<(), DBError> {
        ...
        client
            .execute("INSERT INTO entities (id) VALUES ($1)", &[&id])
            .await?;
...
pub mod test {
    ...
    impl DBClient for MockDBClient {
        async fn insert_entity(&self, _: Uuid) -> Result<(), DBError> {
            self.insert_entity.lock().await.take().unwrap()
        }

        async fn get_entity(&self, _: Uuid) -> Result<Entity, DBError> {
    ...
    {
        let mut entity = DBEntity { id: fixture_uuid() };
        func(&mut entity);
        entity
    }
    ...
    async fn test_get_entity_not_found() {
        ...
        run_db_test(vec![], |db_client| async move {
            let got_result = db_client.get_entity(id).await;
